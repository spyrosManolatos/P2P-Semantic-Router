...
class ChordNode:
    ...
    RPC layer instead of xmlrpc.server/xmlrpc.client. Same routing, replication
    and clustering logic as architectures.clustered_dht.node (K-Means clusters
    placed via SHA-1, destroying ring locality between semantically adjacent
    clusters), with the RPC transport swapped to a persistent, connection-pooled
    httpx.AsyncClient and an asyncio event loop dispatch instead of a
    thread-per-request xmlrpc.server model."""
    ...
    def __init__(self, ip: str, port: int, m: int = None, r: int = None, dataset: str = "kaggle"):
        ...
        # Failure-detector hysteresis (see async_semantic_router.node): only
        # declare a successor dead after this many CONSECUTIVE missed pings, so
        # a momentarily-busy event loop (20 vnodes/container) can't cause a
        # false removal -> successor churn -> finger non-convergence.
    ...
    def _vectorize_and_find_centroids(self, text: str, nprobe: int = 1) -> List[int]:
        ...
        cluster IDs (best ± radius) for nprobe fanout -- identical selection strategy to the
        semantic router. This keeps the only architectural difference between the two
        get_cluster_hash's placement method (SHA-1 here vs. linear mapping), not cluster
        selection."""
    ...
    def get_hash(self, val: str) -> int:
        ...
        not peer addresses -- use get_addr_hash for those). get_cluster_hash
        deliberately keeps using this for its "cluster_{id}" scatter key."""
        if not val:
            return 0
    ...
    async def get_similar_courses(self, course_json: str, nprobe: int = 1, return_hops: bool = False, return_trace: bool = False) -> Any:
        ...
        # trace records one lookup (hops + path) per cluster.
        trace = None
        if return_trace:
            trace = {
        ...
        # 1. Resolve successors and deduplicate targets -- SHA-1 scatters clusters,
        # so unlike the semantic router there is no adjacent-forwarding shortcut:
        # every cluster requires its own fresh O(log N) find_successor lookup.
        for cluster_id in top_clusters:
            cluster_hash = self.get_cluster_hash(cluster_id)
            if return_trace:
                target_node, hops, lookup_path = await self.find_successor_traced(str(cluster_hash))
                ...
                target_node, hops = await self.find_successor_with_hops(str(cluster_hash))
                if target_node not in unique_nodes:
                    unique_nodes[target_node] = []
        ...
        for target_node, cluster_ids in unique_nodes.items():
            if target_node == self.address:
                for cid in cluster_ids:
                    ...
                    results.extend(await self.retrieve_local(cid))
            else:
                print(f" └──> [{self.address}] Routing network request for clusters {cluster_ids} to {target_node}...")
                try:
                    for cid in cluster_ids:
                        results.extend(await self._rpc(target_node, "retrieve_local", cid))
                except Exception as e:
                    print(f"[{self.address}] Failed to fetch clusters {cluster_ids} from {target_node}: {e}")

        unique_results = list(set(results))

        # Batched cosine ranking (see async_semantic_router.node for rationale):
        # one numpy matmul instead of a pure-Python 1000-dim dot per candidate,
        # so high-nprobe queries with large candidate sets don't blow the timeout.
    ...
    async def stabilize(self):
        ...
        if not alive_successor:
            # Only fall back to finger-based recovery once the list is actually
            # empty (all successors confirmed dead). Transient misses keep the
            # current topology and retry next tick instead of churning it.
            if self.successors:
                return
            found_alive = False
